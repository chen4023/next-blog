...
export function PostCard({ post }: PostCardProps) {
  return (
    <Card className="group bg-card/50 border-border/40 hover:cursor-pointer hover:border-primary/20 overflow-hidden border backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:shadow-lg">
      {post.coverImage && (
        ...
        <div className="text-muted-foreground mt-6 flex items-center gap-x-4 text-sm">
          ...
          {post.date && (
            <div className="flex items-center gap-1.5">
              ...
              <time>{formatDateSimple(post.date)}</time>
            </div>
          )}
        </div>
      </CardContent>
